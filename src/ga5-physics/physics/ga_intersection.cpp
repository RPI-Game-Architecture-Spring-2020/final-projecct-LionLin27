...
bool separating_axis_test(const ga_shape* a, const ga_mat4f& transform_a, const ga_shape* b, const ga_mat4f& transform_b, ga_collision_info* info)
{
	...
	for (uint32_t i = 0; i < axes.size(); ++i)
	{
		...
		ga_vec3f project_a =
			oobb_a._half_vectors[0].project_onto_abs(axis) +
			oobb_a._half_vectors[1].project_onto_abs(axis) +
			oobb_a._half_vectors[2].project_onto_abs(axis);
		ga_vec3f project_b =
			oobb_b._half_vectors[0].project_onto_abs(axis) +
			oobb_b._half_vectors[1].project_onto_abs(axis) +
			oobb_b._half_vectors[2].project_onto_abs(axis);

		// Find the min and max points for the full projections of a and b.
		float min_a = oobb_a._center.dot(axis) - project_a.mag();
		float max_a = oobb_a._center.dot(axis) + project_a.mag();
		float min_b = oobb_b._center.dot(axis) - project_b.mag();
		float max_b = oobb_b._center.dot(axis) + project_b.mag();
