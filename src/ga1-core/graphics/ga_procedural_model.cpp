...
float inline g_det2(float a, float b, float c, float d) {
	return a * d - b * c;
}
...
float inline g_det3(
	...
	return
		a1 * g_det2(b2, b3, c2, c3)
		- b1 * g_det2(a2, a3, c2, c3)
		+ c1 * g_det2(a2, a3, b2, b3);
}

// ADAPTED DIRECTLY FROM CUTLER'S HW3 CODE
ga_vec3f g_getNormal(const ga_vec3f& a, const ga_vec3f& b, const ga_vec3f& c) {
	ga_vec3f v12 = b;
	v12 -= a;
	ga_vec3f v23 = c;
	v23 -= b;
...
float g_getDistanceToTri(const ga_vec3f& a, const ga_vec3f& b, const ga_vec3f& c,
	const ga_vec3f& p, const ga_vec3f& dir) {

	ga_vec3f normal = g_getNormal(a, b, c);
...
float g_getInternalDistance(const std::vector<ga_vertex>& verts, const ga_vertex& point) {
	...
	for (size_t v = 0; v < verts.size(); v += 3) {
		bestDistance = std::min(
			g_getDistanceToTri(verts[v]._position, verts[v + 1]._position, verts[v + 2]._position, p, n),
			bestDistance);
	}
	return bestDistance;
}

void g_getAllInternalDistances(const std::vector<ga_vertex>& verts, std::vector<float>& dists) {
	for (const ga_vertex& v : verts) {
		dists.push_back(g_getInternalDistance(verts, v));
	}
}
...
void generate_sphere(int prec, struct ga_model* model, bool genInternalDists) {
	int numVertices;
	int numIndices;
	std::vector<int> indices;
	std::vector<ga_vec3f> vertices;
	...
	if (genInternalDists) {
		...
		g_getAllInternalDistances(model->_vertices, dists);
		for (int i = 0; i < dists.size(); ++i) {
			model->_vertices[i]._internal_distance_n = dists[i];
			std::cout << model->_vertices[i]._internal_distance_n << std::endl;
		}
	}
}

void generate_torus(float inner, float outer, int prec, struct ga_model* model, bool genInternalDists)
{
	int numVertices;
	int numIndices;
